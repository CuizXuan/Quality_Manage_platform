<template>
  <div class="env-manage">
    <div class="toolbar">
      <h2 class="page-title">
        <span class="prompt">&gt;</span> ENVIRONMENT
      </h2>
      ...
      <div v-for="env in envStore.environments" :key="env.id" class="env-card panel" :class="{ default: env.is_default }">
        <div class="env-header">
          <div class="env-name">
            ...
            {{ env.name }}
            <span v-if="env.is_default" class="badge">DEFAULT</span>
          </div>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="showModal = false">
      <div class="modal">
        <div class="modal-header">
          <h3>{{ editing ? '// EDIT ENV' : '// NEW ENV' }}</h3>
        ...
        <div class="form-body">
          ...
          <div class="form-group">
            ...
            <div v-for="(v, k, i) in form.variables" :key="i" class="var-input-row">
              <input v-model="form.variables[i].key" placeholder="KEY" />
              <span class="equals">=</span>
              <input v-model="form.variables[i].value" placeholder="VALUE" />
              <button class="icon-btn danger" @click="form.variables.splice(i, 1)">×</button>
            </div>
            <button class="btn" @click="form.variables.push({ key: '', value: '' })">+ ADD VAR</button>
          </div>
        </div>
        <div class="form-actions">
          <button class="btn" @click="showModal = false">CANCEL</button>
          <button class="btn primary" @click="saveEnv">SAVE</button>
        </div>
      </div>
    </div>
  </div>
